...
names(allcorslv2)<- names(allphys)


saveRDS(allcorslv2,paste0(pathP,"allcors_LV2.RDS"))
...
the_mbs<- readRDS(file=paste0(pathP,"lv2_MBs.RDS"))
...
pbs_Graphs<- fmb_Graphs<- mb_Graphs<- vector("list",length=length(freq_mbs))


for(i in 1:length(allcorslv2)){
    rngraph <-renameNetworks(allcorslv2[[i]],allphys[[i]])
...
for(i in 1:length(the_mbs)){
    rngraph <-renameNetworks(the_mbs[[i]],allphys[[i]],type="mb")
    graph_list1 <-network_mb(rngraph,allphys[[i]])
...
for(i in 1:length(fmb_Graphs)){
  ...
  rngraph <-renameNetworks(freq_mbs[[i]],allphys[[i]],type="mb")
  graph_list1 <-network_mb(rngraph,allphys[[i]])
  }
...
for(i in 1:length(pbs_Graphs)){
  ...
  rngraph <-renameNetworks(freq_pbs[[i]],allphys[[i]],type="Residual")
  graph_list1 <-network_emp(rngraph,allphys[[i]])
  }
  pbs_Graphs[[i]]<- graph_list1

}

names(precisionGraphslv2)<-names(pbs_Graphs)<- names(mb_Graphs)<- names(fmb_Graphs)<- names(allphys)
...
names(merged_freqs)<- names(merged_brls)<- names(allphys) 
...
names(merged_graphs2)<-names(allphys)
...
rownames(merged_d3)<-colnames(merged_d3)<- names(allphys)



baseGraphs<- list(mb_Graphs,fmb_Graphs,pbs_Graphs,precisionGraphslv2)
baseAdjs<- list(the_mbs,freq_mbs,freq_pbs,allcorslv2)
names(baseGraphs[[1]])<-names(baseGraphs[[2]])<-names(baseGraphs[[3]])<-names(baseGraphs[[4]])<-names(allphys)
names(baseAdjs[[1]])<-names(baseAdjs[[2]])<-names(baseAdjs[[3]])<-names(baseAdjs[[4]])<-names(allphys)
...
msdf<-bmb<-vamb<-vabs<-bci<- vector("list",length=length(allphys))
for(i in 1:length(allphys )){
  tryCatch({g1<- graph_to_data_frame(baseAdjs[[1]][[i]],allphys[[i]],type="mb")},error=function(e)NA)
 bmb[[i]]<- g1
 tryCatch({g2<- graph_to_data_frame(baseAdjs[[2]][[i]],allphys[[i]],type="mb")},error=function(e)NA)
 vamb[[i]]<- g2
 tryCatch({g3<- graph_to_data_frame(baseAdjs[[3]][[i]],allphys[[i]],type="bs")},error=function(e)NA)
 vabs[[i]]<- g3
 tryCatch({g4<- graph_to_data_frame(baseAdjs[[4]][[i]],allphys[[i]],type="boral")},error=function(e)NA)
 bci[[i]]<-g4

 msdf[[i]]<- rbind(g1,g2,g3,g4)
...
notstringent_adj<- lapply(msdf.2,function(x){
...
stringent<- lapply(merged_graphs3,function(x){
  if(any(is.na(x))==FALSE){
  x2<-delete_vertices(x, degree(x)==0)
  return(x2)}})

theEnsembleGraphs<- list(stringent,notstrn_gs2)
